...
static void snapshot_writer_thread(void *arg) {
    ...
    printf("Starting zero-pause RDB snapshot generation %" PRIu64 "\n",
           ctx->snapshot_gen);
    ...
    // Write CRC footer
    fwrite(&ctx->crc, sizeof(ctx->crc), 1, ctx->rdb_file);
    ...
    printf("Completed zero-pause snapshot: %" PRIu64 " entries, CRC: %08x\n",
...
void ZeroPauseRDB_snapshot(void) {
    ...
    printf("Zero-pause snapshot triggered (generation %" PRIu64 ")\n",
