...
void http_server_init(App* app, int port) {
    ...
    // CRITICAL FIX: Enable SO_REUSEPORT BEFORE binding
//    if (enable_reuseport(server) < 0) {
//        fprintf(stderr, "WARNING: SO_REUSEPORT not available, multi-worker mode disabled\n");
//        // Continue anyway - single worker will work
//    }
